...
def mix_file():
    ...
    if request.method == 'POST':
        ...
        params={
            ...
            'result': "static/Uploads/" +   result_filename ,
        }
...
@app.route('/selfie_path', methods=['GET', 'POST'])
def selfie_file():
    data = {"success": False}
    if request.method == 'POST':
        image=request.form["image"]
        ...
        global file_Saved
        file_Saved = 'static/Uploads/'+ str(uuid.uuid4()) + ".jpeg"
        ...
        global graph
        with graph.as_default():
                results = generate_caption(image_path=file_Saved)
                results=results.strip("eeee")
                ...
                return render_template('caption_success.html', **params_selfie)
